...
//받은 position 수많큼 객체를 생성해서 JSON 배열로 리턴
    ...
    if(id==[]){
        if(password==[]){
            results.push({id:2,name:0,trust:0,emotion:0});
            console.log(results);
            res.json(results);
        }
        else{
            results.push({id:3,name:0,trust:0,emotion:0});
            console.log(results);
            res.json(results);
        }
    }
    else if(id!=[]&&password==[]){
        results.push({id:4,name:0,trust:0,emotion:0});
        console.log(results);
        res.json(results);
    }
    else{
        ...
        if (!err)
        {
            for(var i=0;i<rows.length;i++){
                if(rows[i].id==id&&rows[i].password==password){
                    console.log('Login');
                    login=1;
                    results.push({id:1,name:rows[i].name,trust:rows[i].trust,emotion:rows[i].emotion});
                }
            }
            if(login==0){
                console.log('Not Login');
                results.push({id:0,name:0,trust:0,emotion:0});
            }
            console.log(results);
            res.json(results);
        }
    ...
    connection.query('Insert into chattingRoomList(category, agemin, agemax, sex, trust, title) values(?, ?, ?, ?, ?, ?)',[category, agemin, agemax, sex, trust, roomname], function (err, result) {
        ...
        connection.query('Insert into chattingRoomInfo(title, maintext, id, category, emotion, like1, roomnum) values(?, ?, ?, ?, ?, ?, ?)',[roomname, maintext, user_id, category, emotion, '0', 0], function(err, rows1, fields) { //id정보로 채팅방 찾는 기능 추가
            if (!err)
            {
                ...
                results.push({id:0,userkey:userkey});
                console.log(results);
                res.json(results);
            }
            else
        ...
        if (!err)
        {
            for(var i=0;i<rows.length;i++){
                if(rows[i].title==chattingroomid){
                    results.push({id:rows[i].maintext,userkey:rows[i].id});
                    res.json(results);
                }
            }
        }
            ...
            for(var i=0;i<title.length;i++){
                for(var j=0;j<rows.length;j++){
                    if(title[i]==rows[j].title){
                        results.push({id:rows[j].like1,name:rows[j].title,trust:rows[j].category,emotion:rows[j].emotion});
                    }
                }
            }
            console.log(results);
...
app.get('/userkey/:userkey', function(req, res) {
    ...
    connection.query('SELECT * from userList', function(err, rows, fields) {
        ...
        {
            for(var i=0;i<rows.length;i++){
                if(rows[i].id==id&&rows[i].password){
                    userkey1=rows[i].userkey;
                    results.push({id:userkey1});
                    res.json(results);
                }
            }
        }
...
app.get('/usertitle/:usertitle', function(req, res) {
    ...
    connection.query('SELECT * from chattingRoomInfo', function(err, rows, fields) {
        if (!err)
        {
            for(var i=0;i<rows.length;i++){
                if(userid==rows[i].id){
                    results.push({id:rows[i].title,name:rows[i].category,trust:rows[i].emotion,emotion:rows[i].like1});
                }
            }
            console.log(results);
            res.json(results);
        ...
        if (!err)
        {
            for(var i=0;i<rows.length;i++){
                if(profile==rows[i].userkey){
                    results.push({id:rows[i].emotion,title:rows[i].userkey,maintext:rows[i].trust});
                    res.json(results);
                }
            }
        }
        ...
        if (!err)
        {
            for(var i=0;i<rows.length;i++){
                if(title==rows[i].title){
                    results.push({id:rows[i].chatting,title:rows[i].userkey,maintext:rows[i].emotion});
                }
            }
            res.json(results);
        }
        ...
        if (!err)
        {
            for(var i=0;i<rows.length;i++){
                if(chattingnum==rows[i].title){
                    results.push({id:rows[i].roomnum});
                }
            }
            res.json(results);
        }
